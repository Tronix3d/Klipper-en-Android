...
if [ ! -c "/dev/pts/0" ] || [ ! -c "/dev/pts/1" ] || [ ! -c "/dev/pts/2" ] || [ ! -c "/dev/pts/3" ] || [ ! -c "/dev/pts/4" ]
then
...
inotifywait -m /dev -e create |
  ...
  do
    ([ "\$file" = "0" ] && chmod 777 /dev/pts/0) || ([ "\$file" = "1" ] && chmod 777 /dev/pts/1) || ([ "\$file" = "2" ] && chmod 777 /dev/pts/2) || ([ "\$file" = "3" ] && chmod 777 /dev/pts/3) || ([ "\$file" = "4" ] && chmod 777 /dev/pts/4)
  done
EOF
sudo chmod +x "$TTYFIX"
